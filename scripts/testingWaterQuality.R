# 
# geometry <- "data/raw/county.gpkg"
# processingLevel <- "county"
# version <- 1 
# overwrite <- TRUE
# pacman::p_load(dplyr, sf, lubridate)
# 
# shp <- sf::st_read("data/raw/CountyDrinkingWaterViolations_03252022.shp")
getDrinkingWater <- function(geometry, processingLevel,version, overwrite){
  ...
  if(file.exists(file) & isFALSE(overwrite)){
    ...
    allPWS <- read.csv("data/raw/drinkingWater/AllPWS_CommunityNTNC.csv", stringsAsFactors = F) %>%
    ...
    v <- read.csv("data/raw/drinkingWater/Violations2010_2020.csv", stringsAsFactors = F)
    ...
    Pb <- read.csv("data/raw/drinkingWater/Lead90th.csv", stringsAsFactors = F) # all Pb results
